...
| **Energy calculations** | **GET /energy-calculations/export?site_id=** — JSON for LLM-assisted energy specs (includes `calc_types` + `energy_calculations`). **PUT /energy-calculations/import** — upsert by `(site_id, external_id)`. CRUD: **GET/POST/PATCH/DELETE /energy-calculations**. See [AI-assisted energy calculations](../modeling/ai_assisted_energy_calculations). |
