...
| **BACnet** | **GET /bacnet/gateways** — one entry describing the embedded rusty-bacnet driver. **POST /bacnet/server_hello** — driver config echo. **POST /bacnet/whois_range**, **POST /bacnet/point_discovery**, **POST /bacnet/point_discovery_to_graph** — WhoIs + object enumeration; discovery-to-graph writes `:bacnet_device` / `:bacnet_object` nodes into SeleneDB. **POST /bacnet/read_property**, **POST /bacnet/read_multiple**, **POST /bacnet/write_property** — direct property reads/writes (priority 1-16 required on write). |
