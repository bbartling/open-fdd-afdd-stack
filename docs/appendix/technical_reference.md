...
| `OFDD_APP_VERSION` | 2.0.16 | Fallback when package metadata missing (`PlatformSettings`; keep in sync with root `pyproject.toml` / `frontend/package.json`). |
...
With DB and API reachable and `OFDD_ONBOARD_API_KEY` set:
