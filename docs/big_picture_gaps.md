...
`POST /data-model/reset` does **not** mean "wipe everything."
It means:
...
Suggested improvement:
- generate durable overnight reports by default

### 6. Container log viewing should become part of the testing story

The new container-log feature is not just an admin convenience.
It should support:
...
- **#73** — `Phase 2 – Stack Security Hardening (DB, Caddy, Secrets)`
  - <https://github.com/bbartling/open-fdd/issues/73>

This should remain in the background context for future deployment and production-readiness work.

Also relevant:
- **#44** — `Test Caddy. Integrate and Define Secure Coding Best Practices`
  - <https://github.com/bbartling/open-fdd/issues/44>
