...
| **BACnet from config UI** | The BACnet driver is embedded (rusty-bacnet) in the `api` and `bacnet-scraper` containers. `OFDD_BACNET_INTERFACE` / `OFDD_BACNET_PORT` / `OFDD_BACNET_BROADCAST_ADDRESS` configure the bind; `--bacnet-address` on bootstrap writes `OFDD_BACNET_INTERFACE`. On the Docker host: `./scripts/bootstrap.sh --verify` confirms `/bacnet/server_hello` returns driver config. Restart **api** and **bacnet-scraper** after env changes. |
