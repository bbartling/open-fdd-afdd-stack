...
| **BACnet from config UI** | API container must reach diy-bacnet-server via **`OFDD_BACNET_SERVER_URL`** (Compose default **`http://host.docker.internal:8080`**; **`OFDD_BACNET_ADDRESS`** / **`--bacnet-address`** is BACnet/IP UDP bind only, not the HTTP URL). On the Docker host: **`./scripts/smoke_bacnet_api_to_gateway.sh`** to confirm the API→gateway hop. Hairpin / LAN URL: [OpenClaw, Docker BACnet, and human data modeling](openclaw_bacnet_docker_and_human_modeling). Restart **api** (and **bacnet-scraper** if you change `.env`). |
