...
## 1c2) BACnet + OpenClaw (embedded rusty-bacnet driver)

The BACnet driver is embedded (rusty-bacnet) in both the `api` and `bacnet-scraper` containers. There is no separate gateway container and no JSON-RPC middle layer. The `bacnet-scraper` runs with `network_mode: host` so the driver can bind UDP/47808 on the host NIC that reaches the BAS.

**Practical OpenClaw path:** call `http://openfdd_api:8000/bacnet/*` with the API key. The API is on the Docker bridge, so OpenClaw (after `docker network connect openfdd_default <claw>`) can reach it the same way as any other bridge peer. Endpoints of interest: `/bacnet/whois_range`, `/bacnet/point_discovery_to_graph`, `/bacnet/read_property`, `/bacnet/write_property`.

**Operator smoke:** `./scripts/bootstrap.sh --verify` — checks the API container can answer `POST /bacnet/server_hello` (driver config echo). A red dot here means the API or the driver failed to load, not that the BAS is unreachable.

For the full checklist, agent curls, and the React export → tag → import flow, see **[OpenClaw, Docker BACnet, and human data modeling](howto/openclaw_bacnet_docker_and_human_modeling)**.
