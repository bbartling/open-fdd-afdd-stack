...
- **Upstream markdown (bootstrap `--with-mcp-rag` only):** shallow sparse `git` clones under `stack/mcp-rag/.vendor-docs/<repo>/docs` from [open-fdd/docs](https://github.com/bbartling/open-fdd/tree/master/docs), [rusty-bacnet/docs](https://github.com/jscott3201/rusty-bacnet/tree/main/docs), and [easy-aso/docs](https://github.com/bbartling/easy-aso/tree/master/docs). Those trees are indexed with tags like `upstream:open-fdd` and stable `source` paths such as `open-fdd/docs/rules/overview.md` (not the full repositories).

Never edit index artifacts as source-of-truth documentation.

**OpenClaw + BACnet + UI modeling:** keep [OpenClaw, Docker BACnet, and human data modeling](../howto/openclaw_bacnet_docker_and_human_modeling) accurate (rusty-bacnet embedded in `bacnet-scraper` + `api`, `network_mode: host`, BACnet API reachable through `openfdd_api:8000/bacnet/*`). After editing that page, rebuild the index (`python scripts/build_mcp_rag_index.py`) and `docker restart openfdd_mcp_rag` so retrieval matches git.
