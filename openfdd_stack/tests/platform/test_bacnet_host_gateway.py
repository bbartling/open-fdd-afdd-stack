...
def test_host_http_url_from_bacnet_address_env_none(
...
def test_host_http_url_from_bacnet_address_env_parses(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_bacnet_rpc_base_candidates_plain_url_no_extra():
    assert m.bacnet_rpc_base_candidates("http://192.168.1.10:8080") == [
        "http://192.168.1.10:8080"
    ]


def test_bacnet_rpc_base_candidates_adds_from_env_address(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_bacnet_rpc_base_candidates_adds_gateway_when_host_docker(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(m, "linux_default_ipv4_gateway", lambda: "172.20.0.1")
    assert m.bacnet_rpc_base_candidates("http://host.docker.internal:8080") == [
...
def test_bacnet_rpc_base_candidates_dedupes_lan_same_as_gateway_swap(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
