...
def test_timeseries_purge_site_deletes_site_rows_only():
    ...
    with (
        patch("openfdd_stack.platform.api.timeseries.get_conn", return_value=conn),
        patch(
            "openfdd_stack.platform.api.timeseries.resolve_site_uuid",
...
def test_timeseries_purge_404_when_site_missing():
    with patch(
        "openfdd_stack.platform.api.timeseries.resolve_site_uuid", return_value=None
    ):
